...
// GetProcessedBlocksFunc is any function that can take an array of CIDs and return a
// channel of incoming processed blocks.
type GetProcessedBlocksFunc func(context.Context, map[cid.Cid]dp.FunctionCodeBlock) (<-chan blocks.Block, error)

// SyncGetProcessedBlock takes a block cid and an async function for getting several
// blocks that returns a channel, and uses that function to return the
// processed block syncronously.
func SyncGetProcessedBlock(p context.Context, k cid.Cid, functionCode dp.FunctionCodeBlock, gb GetProcessedBlocksFunc) (blocks.Block, error) {
	if !k.Defined() {
	...
	m := make(map[cid.Cid]dp.FunctionCodeBlock)
	m[k] = functionCode

	promise, err := gb(ctx, m)
	if err != nil {
		return nil, err
	}
...
// WantFunc is any function that can express a want for set of blocks.
type WantProcessedFunc func(context.Context, map[cid.Cid]dp.FunctionCodeBlock)

// AsyncGetProcessedBlock take a set of block cids, a pubsub channel for incoming
// blocks, a want function, and a close function, and returns a channel of
// incoming blocks.
func AsyncGetProcessedBlock(ctx context.Context, sessctx context.Context, processFunctions map[cid.Cid]dp.FunctionCodeBlock, notif notifications.PubSub,
	want WantProcessedFunc, cwants func([]cid.Cid)) (<-chan blocks.Block, error) {

	keys := getCidFromMap(processFunctions)
...
func getCidFromMap(blockFunctions map[cid.Cid]dp.FunctionCodeBlock) []cid.Cid {
	keys := make([]cid.Cid, 0, len(blockFunctions))
	for k := range blockFunctions {
		keys = append(keys, k)
	}
	return keys
}
